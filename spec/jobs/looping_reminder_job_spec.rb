...
RSpec.describe LoopingReminderJob do
  ...
  let(:reminder) { create(:looping_reminder, user: user, interval_minutes: 5, message: "have you done the dishes?", active: true) }
    ...
    context "when the looping reminder is active" do
      ...
      it "caches audio expiring exactly at 5 minutes" do
        ...
        travel_to(base_time + 4.minutes + 59.seconds) do
          expect(cache_store.read("voice_alert_#{user.id}_tok1")).not_to be_nil
        end

        # Gone after 5 minutes
        travel_to(base_time + 5.minutes + 1.second) do
          expect(cache_store.read("voice_alert_#{user.id}_tok1")).to be_nil
        end
      end
